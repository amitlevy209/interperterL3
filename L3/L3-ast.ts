...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp| ClassExp;//__________________________
...
//L3' 
export type ClassExp = {tag: "ClassExp"; fields: VarDecl[]; methods: Binding[]; }//__________________________
...
export const makeClassExp = (fields: VarDecl[], methods: Binding[]): ClassExp =>
    ({tag: "ClassExp", fields: fields, methods: methods});//__________________________
...
//L3'
export const isClassExp = (x: any): x is ClassExp => x.tag === "ClassExp";//__________________________
...
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isLitExp(x) || isLetExp(x)|| isClassExp(x);//__________________________
...
export const parseL3SpecialForm = (op: Sexp, params: Sexp[]): Result<CExp> =>
    ...
    op === "class" ?
        isNonEmptyList<Sexp>(params) ? parseClassExp(params) ://__________________________
...
const parseClassExp = (exp: Sexp[]): Result<ClassExp> => {//__________________________
   if (exp.length !=2 ) {
         return makeFailure("Invalid structure for ClassExp")
   }
   if (exp[0].length == 0){
    return makeFailure("Invalid structure for ClassExp")
   }
   if (!isGoodBindings(exp[1])||!isArray(exp[0]) || !allT(isString, exp[0])){
    return makeFailure("Invalid structure for ClassExp")
   }
   const methodsNames = map(b => b[0], exp[1]);
   const methodsbodyResult = mapResult(parseL3CExp, map(second, exp[1]));
   const bindingsResult = mapv(methodsbodyResult, (methodBody: CExp[]) => zipWith(makeBinding, methodsNames, methodBody));
   return  mapv(bindingsResult, (par: Binding[]) =>
           makeClassExp(map(makeVarDecl,exp[0] as string[]), par));
}
...
const unparseClassExp = (ce: ClassExp)  : string => {//__________________________
    return `(class (${ce.fields.map((f: VarDecl) => f.var).join(" ")}) (${ce.methods.map((m: Binding) => `(${m.var.var} ${unparseL3(m.val)})`).join(" ")}))`;
}
...
export const unparseL3 = (exp: Program | Exp): string =>
    ...
    isClassExp(exp)? unparseClassExp(exp): //__________________________
    exp;
